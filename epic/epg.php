...
class plgVmPaymentEpg extends vmPSPlugin {
    ...
    function __construct(& $subject, $config) {
        ...
        $varsToPush = array(
            'epg_seller_id' => array('', 'char'),
            'epg_secret_word' => array('', 'char'),
    ...
    function _getEpgDetails($method)
    {
        $epgDetails = array(
            'seller_id' => $method->epg_seller_id,
            'secret_word' => $method->epg_secret_word,
    ...
    public function getVmPluginCreateTableSQL() {
        ...
        return $this->createTableSQL('Payment Epg Table');
    }
    ...
    function getTableSQLFields() {
        ...
        $SQLfields = array(
            ...
            'tax_id' => 'smallint(1) ',
            'epg_response' => 'varchar(255)  ',
            'epg_response_order_number' => 'char(20) '
        );
        return $SQLfields;
    }
    ...
    function plgVmConfirmedOrder($cart, $order) {
        ...
        $epgDetails = $this->_getEpgDetails($method);

        if (empty($epgDetails['seller_id'])) {
            vmInfo(vmText::_('VMPAYMENT_EPG_SELLER_ID_NOT_SET'));
            return false;
        }

        $testReq = $method->debug == 1 ? 'YES' : 'NO';
        $post_variables = array(
            "sid" => $epgDetails['seller_id'],
            ...
            "cart_order_id" => vmText::_('VMPAYMENT_EPG_ORDER_NUMBER') . ': ' . $order['details']['BT']->order_number,
        ...
        $dbValues['epg_custom'] = $return_context;
        ...
        $html .= '<form action="' . $epgDetails["env"] . '/checkout/purchase' . '" method="post" id="vmPaymentForm" >';
        foreach ($post_variables as $name => $value) {
            $html.= '<input type="hidden" name="' . $name . '" value="' . htmlspecialchars($value) . '" />';
        }
        $html.= '<input type="submit" id="epg_submit" class="vm-button-correct" style="display:none" value="' . vmText::_('VMPAYMENT_EPG_BUTTON_MESSAGE') . '" />';
        $html.= '</form></div><br />';
        ...
        vmJsApi::addJScript('vm.paymentFormAutoSubmit', '
  			jQuery(document).ready(function($){
  				...
  				var msg="'.vmText::_('VMPAYMENT_EPG_REDIRECT_MESSAGE').'";
    			...
    			window.setTimeout("jQuery(\'#epg_submit\').show();", 5000);
			})
		');
    ...
    function plgVmOnPaymentResponseReceived(&$html) {
        ...
        $epg_data = vRequest::getRequest();
        $payment_name = $this->renderPluginName($method);

        
        
        if (!empty($epg_data)) {
            vmdebug('plgVmOnPaymentResponseReceived', $epg_data);
            $order_number = $epg_data['merchant_order_id'];
            $return_context = $epg_data['custom'];
            ...
            if ($virtuemart_order_id) {
                ...
                $order['order_status'] = $this->_getPaymentStatus($method, $epg_data['key'], $epg_data['order_number'], $customer_total);
                $order['comments'] = vmText::sprintf('VMPAYMENT_EPG_PAYMENT_STATUS_CONFIRMED', $order_number);
                // send the email ONLY if payment has been accepted

                $nb_history = count($order['history']);
                if ($order['history'][$nb_history - 1]->order_status_code != $order['order_status']) {
                    $this->_storeEpgInternalData($method, $epg_data, $virtuemart_order_id);
                    $this->logInfo('plgVmOnPaymentResponseReceived, sentOrderConfirmedEmail ' . $order_number, 'message');
                    $order['virtuemart_order_id'] = $virtuemart_order_id;
                    $order['comments'] = vmText::sprintf('VMPAYMENT_EPG_EMAIL_SENT');
                    $modelOrder->updateStatusForOneOrder($virtuemart_order_id, $order, true);
                }

               
                $html = $this->_getPaymentResponseHtml($epg_data, $payment_name);
    ...
    function _storeEpgInternalData($method, $epg_data, $virtuemart_order_id) {
        ...
        $post_msg = '';
        foreach ($epg_data as $key => $value) {
            $post_msg .= $key . "=" . $value . "<br />";
            $table_key = 'epg_response_' . $key;
        ...
        $response_fields['epgresponse_raw'] = $post_msg;
        $return_context = $epg_data['custom'];
        $response_fields['order_number'] = $epg_data['merchant_order_id'];
    ...
    function _getPaymentStatus($method, $key, $order_number, $total) {
        $epgDetails = $this->_getEpgDetails($method);
        $compare_string = $epgDetails['secret_word'] . $epgDetails['seller_id'] . $order_number . $total;
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id) {
		...
		$html .= $this->getHtmlRowBE ('EPG_ORDER_NUMBER', $paymentTable->epg_response_order_number);
		$html .= '</table>' . "\n";
		return $html;
    ...
    function _getPaymentResponseHtml($epgTable, $payment_name) {

        $html = '<table>' . "\n";
        $html .= $this->getHtmlRow('EPG_PAYMENT_NAME', $payment_name);
        if (!empty($epgTable)) {
            $html .= $this->getHtmlRow('EPG_ORDER_NUMBER', $epgTable['order_number']);
            $html .= $this->getHtmlRow('EPG_AMOUNT', $epgTable['total'] . " " . $epgTable['currency_code']);
        }
        $html .= '</table>' . "\n";
