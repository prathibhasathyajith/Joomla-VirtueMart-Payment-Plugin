...
//     public function postflight($route, JAdapterInstance $adapter);
    ...
    public function install(JAdapterInstance $adapter)
    {
        // enabling plugin
        $db =& JFactory::getDBO();
        $db->setQuery('update #__extensions set enabled = 1 where type = "plugin" and element = "epg" and folder = "vmpayment"');
        $db->query();
...
//     }
    ...
    public function uninstall(JAdapterInstance $adapter)
    {
        // Remove plugin table
        $db =& JFactory::getDBO();
        $db->setQuery('DROP TABLE `#__virtuemart_payment_plg_epg`;');
        $db->query();
    }
}
